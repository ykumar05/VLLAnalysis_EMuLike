...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ] || [ -z "$6" ] || [ -z "$7" ] || [ -z "$8" ] || [ -z "$9" ]; then
    ...
    echo "   ex: ./createJobsToCondor.sh  runana.C TTJets /cms/multilepton/NTUPLES/2015/MC/TTJets/0000 data year lep era mc flag"
    echo ""
    echo "..exiting!"
    echo ""
    exit 1
...
MAINDIR=`pwd`
SCRIPTDIR=`pwd`
LOGDIR="/home/work/ykumar1/Work/VLLAnalysis_e-muLike/AnalyzerCode/CondorSetup/CondorOutput"
#`pwd`
...
do
  ...
  echo "arguments = $inSampleDir/$ifdir $OUTDIR/\$(Cluster)_${ofile}_data.root $4 $5 $6 $7 $8 $9" >> $condorFile
